...
public class DiscountCardController(IMediator mediator, IFileStorageService fileStorage) : ControllerBase
{
    ...
    {
        var dto = await mediator.Send(new CreateDiscountCardCommand(GetUserId(), body.Name, body.Comment), ct);
    ...
    public async Task<ActionResult<DiscountCardDto>> UpdateCard(int id, [FromBody] UpdateCardRequest body, CancellationToken ct)
        => Ok(await mediator.Send(new UpdateDiscountCardCommand(id, body.Name, body.Comment), ct));
    ...
    public record CreateCardRequest(string Name, string? Comment);
    public record UpdateCardRequest(string Name, string? Comment);
